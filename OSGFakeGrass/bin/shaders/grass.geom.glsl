...
void main() {
  ...
  vec4 colors[9] = 
	...
	if(projCoords.x < -1 || projCoords.x > 1 || projCoords.y < -1 || projCoords.y > 1)
	   return;
	float h = gs_in[0].bladeHeight;//minBladeHeight + (maxBladeHeight - minBladeHeight) * genRan(xyz.xy, 73289.5473661);
	float w = gs_in[0].bladeWidth;//minBladeWidth + (maxBladeWidth - minBladeWidth) * genRan(xyz.xy, 14567.5443867);
	float tilt = gs_in[0].tilt;//(minTilt + (maxTilt - minTilt) * genRan(xyz.xy, 73289.5473661)) * kDegToRad;
	float rotate = gs_in[0].rotate;//(minRotate + (maxRotate - minRotate) * genRan(xyz.xy, 73289.5473661)) * kDegToRad;
  ...
  for(float col = -win;col <= win; col++)
	{
	vsColor = vec4(col,row,0,1);
	vsColor.xy = vsColor.xy * 0.5 + vec2(0.5);
	vsColor = colors[n];
	n++;
	vec3 translate = vec3(col*gridSize, row*gridSize, 0);
	root = gl_in[0].gl_Position.xyz + translate;
	root.z = 0;
	uv = vec2(1, 0);
	position = v0+root;
