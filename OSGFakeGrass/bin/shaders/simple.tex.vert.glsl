...
void main(void)
{
   ...
   //mat4 proj = createOrtho(-0.6,0.6,-0.6,0.6,1,1000);
   //gl_ModelViewProjectionMatrix 
   vec3 bbMin = vec3(-0.5, -0.5, 0);
   vec3 bbMax = vec3(0.5, 0.5, 0.5);
   //vec3 center = vec3(0, 0, 0);

   //vec3 vertexToEye = vec3(1, 0, 0);
   //vec2 tiltRotate = vectorToTiltRotate(vertexToEye);
   //mat4 view = createViewFromTiltRotate(center, tiltRotate.x, tiltRotate.y, length(bbMax));
