...
public:
	...
	std::string DefaultFragmentShader()
	{
		static char fragmentSource[] =
			...
			"void main(void) \n"
			"{\n"
			"    vec4 color = texture2D(texture0, gl_TexCoord[0].xy * u_reps);\n"
			"    gl_FragColor = color;\n"
			"}\n";
...
public:
	...
	RenderSurface(int width, int height, GLenum internalFormat, GLenum sourceFormat, GLenum sourceType, bool allocateImage, bool attachDepth = false)
		: osg::Camera()
	{
		...
		//m_texture->setFilter(osg::Texture2D::MIN_FILTER, osg::Texture2D::LINEAR_MIPMAP_LINEAR);
		//m_texture->setFilter(osg::Texture2D::MAG_FILTER, osg::Texture2D::LINEAR_MIPMAP_LINEAR);
		m_texture->setFilter(osg::Texture2D::MIN_FILTER, osg::Texture2D::LINEAR_MIPMAP_LINEAR);
		m_texture->setFilter(osg::Texture2D::MAG_FILTER, osg::Texture2D::LINEAR_MIPMAP_LINEAR);
		m_texture->setWrap(osg::Texture2D::WRAP_S, osg::Texture2D::REPEAT);
		m_texture->setWrap(osg::Texture2D::WRAP_T, osg::Texture2D::REPEAT);
		m_texture->setUseHardwareMipMapGeneration(true);
		...
		if (attachDepth)
		{
			...
			//m_texture->setFilter(osg::Texture2D::MIN_FILTER, osg::Texture2D::LINEAR_MIPMAP_LINEAR);
			//m_texture->setFilter(osg::Texture2D::MAG_FILTER, osg::Texture2D::LINEAR_MIPMAP_LINEAR);
			m_depthTexture->setFilter(osg::Texture2D::MIN_FILTER, osg::Texture2D::LINEAR_MIPMAP_LINEAR);
			m_depthTexture->setFilter(osg::Texture2D::MAG_FILTER, osg::Texture2D::LINEAR_MIPMAP_LINEAR);
			m_depthTexture->setWrap(osg::Texture2D::WRAP_S, osg::Texture2D::REPEAT);
			m_depthTexture->setWrap(osg::Texture2D::WRAP_T, osg::Texture2D::REPEAT);
			//rtTexture->setDataVariance(osg::Object::DYNAMIC);
			m_depthTexture->setInternalFormat(GL_R32F);
			m_depthTexture->setSourceFormat(GL_R32F);
			m_depthTexture->setSourceType(GL_FLOAT);
			attach(osg::Camera::COLOR_BUFFER1, m_depthTexture.get());
		}
	...
	osg::ref_ptr<osg::Texture2D> DepthTexture() { return m_depthTexture; }
	...
	void Resize(int newWidth, int newHeight)
	{
		...
		if (m_depthTexture)
			m_depthTexture->setTextureSize(newWidth, newHeight);
	}
};
