...
{  
   ...
   vec2 index;
   vec3 pos = v_position;
   vec2 bbMin = vec2(left, bottom);
   if(u_rotateTex)
      pos = (u_localTransform * vec4(v_position,0)).xyz;

   uv = (pos.xy - bbMin) / resolution;
   index = floor(uv);
   uv = fract(uv);
   vec3 center = vec3(bbMin + index * resolution + vec2(0.5,0.5) * resolution, 0);
   ...
   vec4 glposition = projMat * viewMat * vec4(v_position.x, v_position.xyz.y, v_position.xyz.z + depth, 1);
   float adjuestedDepth = glposition.z / glposition.w;
   //gl_FragDepth = ((gl_DepthRange.diff * adjuestedDepth) + gl_DepthRange.near + gl_DepthRange.far) / 2.0;
